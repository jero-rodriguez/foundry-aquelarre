...
import { AQUELARRE } from './helpers/config.mjs';
...
Hooks.once('init', function () {
  ...
  CONFIG.Actor.dataModels = {
    character: models.AquelarreCharacter,
    npc: models.AquelarreNPC
  }
  CONFIG.Item.documentClass = AquelarreItem;
  CONFIG.Item.dataModels = {
    item: models.AquelarreItem,
    feature: models.AquelarreFeature,
    spell: models.AquelarreSpell
  }
  ...
  Actors.registerSheet('aquelarre', AquelarreActorSheet, {
    makeDefault: true,
    label: 'AQUELARRE.SheetLabels.Actor',
  });
  Items.unregisterSheet('core', ItemSheet);
  Items.registerSheet('aquelarre', AquelarreItemSheet, {
    makeDefault: true,
    label: 'AQUELARRE.SheetLabels.Item',
  });
...
async function createItemMacro(data, slot) {
  ...
  const command = `game.aquelarre.rollItemMacro("${data.uuid}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { 'aquelarre.itemMacro': true },
    });
  }
